...
const Notification = () => {
  ...
  // ✅ Define this function BEFORE any use
  const getNotificationInfo = async () => {
    try {
      const response = await axios.get("http://localhost:5001/Notification/NotificationForms");
      setNotificationInfo(response.data);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      !notificationData.draftNotificationNo ||
      !notificationData.draftNotificationDate ||
      !notificationData.finalNotificationNo ||
      !notificationData.finalNotificationDate ||
      !notificationData.dateOfPublication ||
      !notificationData.concernSection ||
      !notificationData.pdfFile
    ) {
      alert("Please fill in all the fields");
      return;
    }
  ...
  const searchHandle = async (event) => {
    ...
    if (key) {
      try {
        const response = await axios.get(`http://localhost:5001/Notification/api/searchNotification/${key}`);
        setNotificationInfo(response.data.length > 0 ? response.data : []);
      } catch (error) {
        console.error("Error fetching search results:", error);
      }
    } else {
  ...
  return (
    <div className="table">
      ...
      <input type="text" className="search-box" placeholder="Search Notifications" onChange={searchHandle} />
      ...
      {!showForm ? (
        <div>
          <table border="1px solid blue">
            ...
            <tbody>
              {Array.isArray(notificationInfo) && notificationInfo.map((info, index) => (
                <tr key={info._id}>
                  <td>{index + 1}</td>
                  <td>{info.title}</td>
                  <td>{info.draftNotificationNo}</td>
                  ...
                  <td>
                    <button className="delete-button" onClick={() => deleteNotificationInfo(info._id)}>Delete</button>
                    <Link to={`/notification/update/${info._id}`}>Update</Link>
                  </td>
                </tr>
              ))}
        ...
        <form onSubmit={handleSubmit} className="form-container">
          ...
          <div>
            <label>Final Notification No and Date:</label>
            <input type="text" name="finalNotificationNo" value={notificationData.finalNotificationNo} onChange={handleChange} placeholder="Enter Final Notification Number" style={{ marginRight: "10px" }} />
            <input type="date" name="finalNotificationDate" value={notificationData.finalNotificationDate} onChange={handleChange} />
          </div>
          <div>
            <label>Date of Publication:</label>
